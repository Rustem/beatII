...
In this paper we will demonstrate a method of controlling process interactions represented by sessions. The formal theory based on session types ensures communication safety by verifying that session implementations of each engaged participant conform to the defined protocol specification. In order to prove the feasibility of this theory we utilized Session-Java, an extension to Java language. It works by specifying the intended process transaction protocol using session types and implementing the interaction using session operations. The conformance of protocol to its implementation is statically verified in compile mode, while the whole communication between peers is validated at runtime, at negotiation stage.

In this paper, first of all, we outline the main issues in Web-services Choreography Description Language discussed by W3C group \cite{ws-critical-overview}, then we describe the main concepts of correctly functioning communication of processes (\textit{conformance}, \textbf{well-formedness}), after that we show the implementation of Cloud scenarios and highlight possible application of session types into a services operating within multiple cloud vendors (cloud integrator).
...
\section{Implementation of Choreography}
\label{sect:chor}
Interacting processes must accomplish the goal of the computation. For this, the processes should have correct functionalities and correct interact with each other. With the interaction becoming more complex, the problems related to specify the interaction of the participants will be harder too, if we still want to do it locally. In addition, it is harder to verify the interaction locally. These are the main motivation under the design of WS-CDL.

Assume that a choreography \textit{C} defines random role \textit{R}, then a process \textit{P} may be considered as implementation of R, \(C \unrhd P\), if

\begin{enumerate}
\item
\textit{P} can execute all communication required by \textit{R} with other roles of \textit{C}, in suitable time with suitable order.
\item
\textit{P} support all the local activities mentioned in \textit{R}.
\end{enumerate}
The paper \cite{chor-essence} defined a small language Chor, a model of simplified WS-CDL, and a simple process language for the description of roles from a local viewpoint. Based on these models, this paper describes the concept of projections, that map a given choreography \textit{C} to a set of role processes.  Using a projection, we will get a set of processes, where each of them represents a role in the choreography. The main problem here is to give an answer on the following question: ``Does process P can play role R defined by choreography C?''.

So a reasonable definition of the implementation of choreography is based on the concepts of projection and local conformance. By projection, it should be considered a procedure which takes a choreography in Chor and delivers a set of processes in the role language, while each of the processes corresponds to a role in choreography. In other words, a projection can partition a choreography \textit{C} into a set of independent processes which realize the behaviour specified by this choreography provided in Eq. \ref{eq:proj}.
\begin{equation}
\label{eq:proj}
[[proj(C, 1) \quad || \dots || \quad proj(C, n)]] = [[C]]
\end{equation}

\section{Implementation of Cloud Scenarios}
\label{sect:impl}

\subsection{Scenario 1: SaaS integration}
\subsection{Scenario 2: Payment and Wallet Recharging}
...
\label{sect:chor}
\subsection{Main Criterias for Faultless Web-services}

\subsection{Choreography Implementation and Conformance}
...
{\sf Helvetica}. The base font size is 10pt. If you use any other font
size, there is no guarantee that the produced document will look nice
or fit into our standard page size.
...
Figures~\ref{fig:easychair-logo}. If the papers are designed for
...
page numbers in as-is. When EasyChair proceedings are processed by
...
\section{Submitting Your Article for EasyChair Proceedings}

This section is intended only for the authors and editors of EasyChair
proceedings. 

When you prepare an article for EasyChair proceedings, it should be
...
anybody can become a low-cost publisher.
...
\begin{itemize}
\item
{\easychair} version 3.1 -- May 2011, several changes intended for
automatic processing by EasyChair.
