...
s.request() & \texttt{begin} \\ \hline 
s.send(m) & $\mathopen{!}\left<M\right>$ \\ \hline
s.receive() & $\mathopen{?}\left(M\right)$ \\ \hline
s.outbranch(L) \{P\} & $\mathopen{!}\left\{L\colon T\right\}$ \\ \hline
s.inbranch() \{case L1: {P1}\dots case Ln:{Pn}\} & $\mathopen{?}\left\{L_1\colon T_1,\dots, L_n\colon T_n\right\}$ \\ \hline
s.outwhile(cond) \{P\} & $\left[T\right]*$ \\ \hline
s.inwhile() \{P\} & $\mathopen{?}\left[T\right]*$ \\ \hline
s.recursion(L) \{P\} & $\texttt{rec}\,L\left[T\right]$ \\ \hline
s.recurse(L) & $\mathopen{\#}L$ \\ \hline
\end{longtable}
